...
@testset "GitHub Issues" begin
    ...
    @testset "593 preserve single-line blocks" begin
        # With `preserve_single_line_blocks` (enabled for MinimalStyle), block constructs
        # written entirely on one source line keep their single-line form.
        unchanged = [
        ...
        # Constructs spanning multiple source lines are still expanded.
        test_format(
            "try\n    isfile(x)\ncatch\n    false\nend\n",
            "try\n    isfile(x)\ncatch\n    false\nend\n",
            MinimalStyle();
        ...
        # The option is off by default: DefaultStyle still expands one-liners.
        test_format(
            "try isfile(x); catch; false end\n",
            "try\n    isfile(x)\ncatch\n    false\nend\n";
            ast=true,
        )
        # ... but can be enabled standalone.
        test_format("if x; y end\n", "if x; y end\n"; preserve_single_line_blocks=true, ast=true)

        # YAS and SciML behavior is unchanged.
        test_format(
            "try isfile(x); catch; false end\n",
            "try\n    isfile(x)\ncatch\n    false\nend\n",
            YASStyle();
    ...
    @testset "1238 block as the first element of a tuple" begin
        # MinimalStyle enables preserve_single_line_blocks (#593), so these single-line
        # blocks are now kept as-is instead of being expanded.
        str_ = "(begin x = 1 end, 1)"
        str1 = "(begin\n    x = 1\nend, 1)"
        str2 = "(begin\n     x = 1\n end, 1)"
        str3 = "(\n    begin\n        x = 1\n    end,\n    1,\n)"
        test_format(str_, str1, DefaultStyle())
        test_format(str_, str_, MinimalStyle())
        test_format(str_, str2, YASStyle())
        test_format(str_, str3, BlueStyle())

        str_ = "(for _ = 1:10; x = 1; end, 1)"
        str = "(for _ = 1:10\n    x = 1\nend, 1)"
        test_format(str_, str, DefaultStyle())
        test_format(str_, str_, MinimalStyle())
